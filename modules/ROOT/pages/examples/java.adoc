...
<8> Here we schedule such that `queryChanges` gets called repeatedly.
<9> Here you can limit the returned changes. The out-commented line would select only node changes and exclude all relationship changes.