...
For example, specifying both `name` and `surname` as a `changesTo` value will only return changes where both `name` *and* `surname` properties have changed within the same transaction.
