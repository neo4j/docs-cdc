...
[WARNING]
====
Restoring your Neo4j database from a backup is likely to cause *loss of data and corrupted data in your CDC application*. Read the following sections carefully if you are using CDC and intend to restore your database from backup.
====
=== Loss of change events
The Neo4j restore procedures replace the existing database with a *new* database.
This database has a new transaction log and any change identifier (cursor) generated from the old transaction log will not be usable with the new database.
Furthermore, care has to be taken that the transaction log retention setting of the restored database matches that of your existing database.

Follow the steps outlined below in order to mitigate this issue.

[NOTE]
The steps outlined here ensure that all change events are transmitted to your CDC application. Depending on your application however, these change events might still be unexpected and cause further issues. Read xref:_unexpected_change_events[the section below], before performing these steps.

. *Set the database into read only mode.* +
This prevents any new changes coming through.

. *Note down the `txLogEnrichment` value of your database.* +
...
Compare the output of `SHOW DATABASES YIELD name, options` with the value noted down in step 2.
...
If the node/relationship is (re-)created in your Neo4j database, your CDC application will see a duplicate node/relationship.
...
If the node/relationship is updated, your CDC application will see an update to a previously deleted entity.
...
Causing the two systems to treat the node differently.
