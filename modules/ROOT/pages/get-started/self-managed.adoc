...
To create a new database with CDC enabled, use the link:https://www.neo4j.com/docs/operations-manual/current/database-administration/standard-databases/create-databases/[`CREATE DATABASE`] Cypher command and set the option `txLogEnrichment` to either `FULL` or `DIFF`.
...
To tweak the CDC mode on an existing database, use the https://www.neo4j.com/docs/operations-manual/current/database-administration/standard-databases/alter-databases/[`ALTER DATABASE`] Cypher command and set the option `txLogEnrichment` to either `FULL` or `DIFF`.
...
To see what value the CDC mode of a database is, use the link:https://neo4j.com/docs/operations-manual/current/database-administration/standard-databases/listing-databases/[`SHOW DATABASES`] Cypher command.
...
.Result
[role="queryresult",options="header",cols="1m,1m"]
|===
| name | options
...
Since CDC information is stored in transaction log entries, the retention period of these logs determines how far back in time your application can query for CDC data..
...
For example, when importing data with the link:https://neo4j.com/docs/operations-manual/current/import/[`neo4j-admin database import`] tool, whether full or incremental, or when loading data with the link:{neo4j-docs-base-uri}/operations-manual/current/backup-restore/restore-dump/[`neo4j-admin database load`] tool, data is written directly to the store without sending anything to the transaction log, so such changes are **not** captured by CDC.
