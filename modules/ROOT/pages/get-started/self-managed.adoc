= CDC on Neo4j DBMS
:page-role: beta enterprise-edition
...
=== Syncing to another Neo4j database
CDC is a suitable tool for copying all or some data change events from one database to another, as well as building software that reacts to data change events in the database.

However, it is not a suitable tool for creating an exact copy of your Neo4j database.
Whilst the change event includes metadata about the transaction, it cannot be used to create identical elements in another Neo4j instance.
