...
Admin users can enable CDC for an Aura database instance using the Aura Console UI.
Non-admin users may view the current enrichment mode, but may not edit it.

To enable or tweak the enrichment mode, access the instance options through the three dots on an instance card and select *Edit CDC Enrichment Mode*.
...
Although it is possible to pause a database that has enrichment mode set to `DIFF` or `FULL`, **enrichment mode gets set to `OFF` when the database is resumed**.
