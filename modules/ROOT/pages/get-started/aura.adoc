...
xref:procedures.adoc/#earliest-change-event[Retrieving the earliest change event] may help you get a feeling for how far back in time your database's transaction logs are retained, although you should be careful in making assumptions based on that: *the rate at which old transactions are pruned from logs depends on the number and size of incoming transactions*, and is thus likely to oscillate over time.
