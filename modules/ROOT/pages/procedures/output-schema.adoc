...
May be different from `authenticatedUser` when using link:https://neo4j.com/docs/operations-manual/current/authentication-authorization/dbms-administration/dbms-impersonate-privileges/[impersonation].
...
See link:{neo4j-docs-base-uri}/operations-manual/{page-version}/clustering/servers/#_showing_servers[Operations Manual -> Clustering -> Managing servers in a cluster] for more information on servers.
