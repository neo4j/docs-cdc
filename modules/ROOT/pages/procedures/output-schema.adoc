...
The changes occurring within a transaction are tracked as change events in the following order:
...
Note that the order of changes in the output is **not guaranteed** to match exactly the order of changes occurring in a transaction.
CDC tracks logical changes only; therefore, if two changes cancel each other (for example, a `DELETE` following a `CREATE` on the same node), none is included in the stream.
