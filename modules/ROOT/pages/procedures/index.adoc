...
From Neo4j 2026.06 when using Cypher 25, the `txCommitTime` field is also returned, representing the timestamp of the last committed transaction.
...
====
.Query
[source, cypher, test-exclude-cols=id,txCommitTime]
----
CALL db.cdc.current()
----
...
////
fails with: `wrong database. Change identifier $`A3V16ZaLlUmnipHLFkWrlA0AAAAAAAAABAAAAAAAAAAA` does not belong to this database.`
////
...
.Result
[role="queryresult",options="header",cols="3m,1m,1m,5a,5a"]
|===
...
.Result
[role="queryresult",options="header",cols="a,a"]
|===
| event | metadata
