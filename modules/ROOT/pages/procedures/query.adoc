...
Even if it gets enabled again, it will not be possible to query the previous change records.
====
...
This value is treated as exclusive and the results of the query will not include the change corresponding to this value.
...
Default is an empty list, which means _all_ changes will be returned without any filtering taking place.
See xref:selectors/selectors.adoc[selectors] for details.
...
This will be same for all of the changes in a single transaction.
