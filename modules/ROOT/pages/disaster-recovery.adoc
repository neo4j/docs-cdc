...
- The xref:get-started/self-managed.adoc#log-retention[transaction log retention policy] is generous enough to accommodate the maximum number of transactions your CDC application may be behind.
...
<.> The new database should have the same name and xref:get-started/self-managed.adoc[transaction log enrichment mode] as the backed-up database.
