...
[WARNING]
====
The recovery procedure works both on single instances and on clusters since Neo4j 2025.04.
It does not work on clusters running versions earlier than 2025.04.

The exact constraint for clusters is that the recovery should happen onto a cluster running Neo4j 2025.04 or later, and the differential backups _after xref:#aggregate[aggregation]_ come from Neo4j 2025.04 or later.
...
Another way of looking at it is to aggregate backup files up until the latest date _before_ the `txCommitTime` of the latest CDC-processed event.
In the example situation, we can aggregate backups until `2025-04-18T06:57:30`.
Because the backup files lack microsecond precisions, we aggregate until the previous file (`2025-04-18T06-57-21`) to be on the safe side.

.Backup aggregation
[source,shell]
----
neo4j@737154f61ca4:/var/lib/neo4j# neo4j-admin backup aggregate --from-path=backups/neo4j/neo4j-2025-04-18T06-57-21.backup
...
[source,shell]
----
neo4j@737154f61ca4:/var/lib/neo4j# ls -l backups/neo4j/
...
The filename given in `seedURI` allows the server to pinpoint a backup chain, but the exact filename has no importance for the restore process.
Use link:https://neo4j.com/docs/operations-manual/current/database-administration/standard-databases/seed-from-uri/#_support_for_seeding_up_to_a_date_or_a_transaction_id[`seedRestoreUntil`] if you want to be specific about the time/transactionID to import up to.


[#restart-cdc]
=== Restart CDC application and query for changes

Once the database is back running again you can xref:procedures/index.adoc#query[query for changes] from the event ID that your CDC application had processed before the database went offline.
